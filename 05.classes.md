...
txt[2] = 'a';//error compilation
char* txt1 = const_cast<char*>(txt);
txt1[2] = 'a';//true

int a=1;
const int& b =a;
const_cast<int&>//недописанно
```
...
struct A
{
    explicit A(int x) {} //запрещает неявное привидение
};
...
class BigInt
{
    static constexpr size_t Size = 256;//constexpr говорит что нужно выполнить при компиляции (т.е. раньше) static для всех объектов одна переменная
    uint8_t data_[Size];
public:
...
BigInt y = 5;
BigInt z = x + y; // ок
BigInt z = x + y + x; // ошибка x+y возвращает ссылку константную,а x constanta 
```
...
class BigInt
{
    friend BigInt operator+(const Int128& x, const BigInt& y);//friend позволяет иметь доступ к полям класса bigint
};
